...
| D-RA-18 | RetroDiffusion dropped from rasa | Same pattern as D-RA-17 — specialty pixel-art tools live in variants, not the foundation. RetroDiffusion moves to limner-pixel; rasa v1 pipeline registry shrinks to 3 (Midjourney, DALL-E, Recraft). Trigger: integration testing surfaced upstream API behavior (server-side hangs on inference requests with valid auth) that isn't worth resolving in rasa's foundation scope when the pixel-art tooling already has a natural home elsewhere |
