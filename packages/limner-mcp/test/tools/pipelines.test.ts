...
describe('limner_generate_recraft', () => {
  ...
  // F4: the generate handler requests b64_json so the bytes re-host to a signed
  // capability URL (like upscale/vectorize) instead of leaking Recraft's
  // ephemeral CDN url. Assert the request asked for bytes AND the result is a
  // re-hosted artifact url, never a recraft.ai host.
  test('requests b64_json and re-hosts generated bytes to a capability URL when delivery is set', async () => {
    const onePixelPng =
      'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR4nGNgYGBgAAAABQABh6FO1AAAAABJRU5ErkJggg==';
    const realFetch = globalThis.fetch;
    ...
    try {
      const result = await client.callTool({
        name: 'limner_generate_recraft',
        arguments: { prompt: 'logo', style: 'realistic_image' },
      });
      expect(result.isError).toBeFalsy();
      // The generations request asked Recraft for inline bytes.
      const body = JSON.parse((fetchSpy.mock.calls[0]![1] as RequestInit).body as string);
      expect(body.response_format).toBe('b64_json');
      // Bytes re-hosted under generated/recraft/<uuid>.png; no recraft.ai leak.
      expect(put).toHaveBeenCalledOnce();
      const key = put.mock.calls[0]![0] as string;
      expect(key).toMatch(/^generated\/recraft\/[0-9a-f-]+\.png$/);
      const content = result.content as Array<{ type: string; text?: string }>;
      expect(content[0]!.type).toBe('text');
      expect(content[0]!.text).toBe(`https://assets.example/artifact/${key}`);
      expect(content[0]!.text).not.toContain('recraft.ai');
    } finally {
      globalThis.fetch = realFetch;
      await close();
    }
  ...
  // F4 robustness: if a style ignores response_format and Recraft returns a
  // hosted url anyway, the transport fetches those bytes through the SSRF guard
  // so the MCP layer still re-hosts — no provider CDN url ever reaches the client.
  test('falls back to fetching a url-only result through the guard, then re-hosts', async () => {
    const onePixelPng =
      'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR4nGNgYGBgAAAABQABh6FO1AAAAABJRU5ErkJggg==';
    const realFetch = globalThis.fetch;
    globalThis.fetch = vi.fn(async (url: unknown) => {
      ...
      if (doh) return doh;
      // The generations call returns a hosted url only (style ignored the flag).
      ...
      // The guarded re-fetch of that hosted url yields the actual png bytes.
